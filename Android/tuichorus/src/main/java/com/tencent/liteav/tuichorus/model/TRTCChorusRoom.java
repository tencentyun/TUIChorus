...
import com.tencent.liteav.tuichorus.model.impl.TRTCChorusRoomImpl;
import com.tencent.rtmp.ui.TXCloudVideoView;

import java.util.List;

public abstract class TRTCChorusRoom {

    /**
     * 获取 TRTCChorusRoom 单例对象
     *
     * @param context Android 上下文，内部会转为 ApplicationContext 用于系统 API 调用
     * @return TRTCChorusRoom 实例
     * @note 可以调用 {@link TRTCChorusRoom#destroySharedInstance()} 销毁单例对象
     */
    public static synchronized TRTCChorusRoom sharedInstance(Context context) {
        return TRTCChorusRoomImpl.sharedInstance(context);
    }

    /**
     * 销毁 TRTCChorusRoom 单例对象
     *
     * @note 销毁实例后，外部缓存的 TRTCChorusRoom 实例不能再使用，需要重新调用 {@link TRTCChorusRoom#sharedInstance(Context)} 获取新实例
     */
    public static void destroySharedInstance() {
        TRTCChorusRoomImpl.destroySharedInstance();
    }
     ...
     * 设置事件回调所在的线程
     *
     * @param handler 线程，TRTCChorusRoom 中的各种状态通知，会派发到您指定的 handler 线程。
     */
