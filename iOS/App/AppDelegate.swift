...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func showMainViewController() {
        ...
        TRTCChorusRoom.shared().login(sdkAppID: Int32(SDKAPPID), userId: userId, userSig: ProfileManager.shared.curUserSig()) { code, message in
            if code == 0 {
                TRTCChorusIMManager.sharedManager().SDKAPPID = Int32(SDKAPPID)
                TRTCChorusIMManager.sharedManager().loadData()
                let listVC = ChorusMainViewController.init()
