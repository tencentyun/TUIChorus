...
class TRTCChorusImageOnlyCell: UICollectionViewCell {
...
class TRTCChorusTopView: UIView {
    
    public let viewModel: TRTCChorusViewModel
    ...
    public func reloadRoomAvatar() {
        roomImageView.kf.setImage(with: URL(string: TRTCChorusIMManager.sharedManager().checkBgImage(viewModel.roomInfo.coverUrl)), placeholder: nil, options: [], completionHandler: nil)
    }
    ...
    private let audienceListCollectionView : UICollectionView = {
        let layout = TRTCChorusAudienceListLayout()
    ...
    private func bindInteraction() {
        ...
        audienceListCollectionView.register(TRTCChorusImageOnlyCell.self, forCellWithReuseIdentifier: "audienceListCell")
        memberAudienceDataSource = viewModel.getRealMemberAudienceList()
        ...
        if viewModel.isOwner {
            ...
            if viewModel.userType == .anchor {
                viewModel.viewResponder?.showAlert(info: (title: .alertToMicoffText, message: ""), sureAction: { [weak self] in
                    ...
                    if TRTCChorusFloatingWindowManager.shared().enableFloatingWindow {
                        guard let rootVC = self.viewModel.rootVC else { return }
                        TRTCChorusFloatingWindowManager.shared().hide(vc: rootVC)
                    } else {
...
class TRTCChorusAudienceListLayout : UICollectionViewFlowLayout {
...
extension TRTCChorusTopView : UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return memberAudienceDataSource.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "audienceListCell", for: indexPath) as! TRTCChorusImageOnlyCell
...
extension TRTCChorusTopView : UICollectionViewDelegate {
